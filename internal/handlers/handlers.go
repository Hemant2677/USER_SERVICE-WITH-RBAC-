...
func UpdateUser(c *gin.Context) {
    ...
    // Extract the token part (no splitting if it's a simple token)
    ...
    // Check if the user has the "update" permission for user resources
    hasPermission := database.HasAccess(userID, "user_update") // Check if the user has the update permission
    if !hasPermission {
        c.JSON(http.StatusForbidden, map[string]any{"error": "You do not have permission to update user information"})
        return
    }
    ...
    var userRequest struct {
        Name  string `json:"name"`
        Age   int    `json:"age"`
        //Email string `json:"email"` // Include email if needed
    }
    ...
    userIDToUpdate, err := strconv.Atoi(paramID) // Convert the paramID to int
    if err != nil {
        c.JSON(http.StatusBadRequest, map[string]any{"error": "Invalid user ID"})
        return
    }

    // Call the database update function with the specified user ID
    err = database.UpdateUserByID(userIDToUpdate, userRequest.Name, userRequest.Age)
    if err != nil {
